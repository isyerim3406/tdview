...
(async () => {
  ...
  try {
    ...
    // Eğer daha önce cookie kaydedildiyse yükle
    let loggedIn = false;
    if (fs.existsSync(cookiesPath)) {
      ...
      if (cookies.length > 0) {
        ...
        try {
          await page.waitForSelector('button[data-name="user-menu-button"], button[data-name="header-user-menu-button"], .userMenu-U2jIw4km', { timeout: 5000 });
    ...
    if (!loggedIn) {
      ...
      // İlk giriş butonuna tıkla (validator çıkması için)
      ...
      // Validator çözmen için 10sn bekle
      ...
      // 10sn daha bekle → login tamamlanması için
      await sleep(10000);
    ...
    // Sayfa yenile (arayüzün temiz açılması için)
